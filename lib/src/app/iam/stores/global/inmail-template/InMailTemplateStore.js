...
var InMailTemplateStore = (_dec = (0, _choerodonBootCombine.store)('InMailTemplateStore'), _dec(_class = (_class2 = function () {
  function InMailTemplateStore() {
    (0, _classCallCheck3['default'])(this, InMailTemplateStore);
    ...
    this.loadMailTemplate = function (_ref, _ref2, _ref3, params, appType, orgId) {
      ...
      if (appType === 'site') {
        return _choerodonBootCombine.axios.get('/notify/v1/notices/letters/templates?page=' + (current - 1) + '&size=' + pageSize + '&' + _queryString2['default'].stringify(queryObj));
      } else {
        return _choerodonBootCombine.axios.get('/notify/v1/notices/letters/templates/organizations/' + orgId + '?page=' + (current - 1) + '&size=' + pageSize + '&' + _queryString2['default'].stringify(queryObj));
      }
    };
    ...
    this.createTemplate = function (data, appType, orgId) {
      var path = appType === 'site' ? '' : '/organizations/' + orgId;
      return _choerodonBootCombine.axios.post('notify/v1/notices/letters/templates' + path, JSON.stringify(data));
    };

    this.deleteMailTemplate = function (id, appType, orgId) {
      var path = appType === 'site' ? '' : '/organizations/' + orgId;
      return _choerodonBootCombine.axios['delete']('/notify/v1/notices/letters/templates/' + id + path);
    };

    this.getTemplateDetail = function (id, appType, orgId) {
      var path = appType === 'site' ? '' : '/organizations/' + orgId;
      return _choerodonBootCombine.axios.get('notify/v1/notices/letters/templates/' + id + path);
    };

    this.updateTemplateDetail = function (id, data, appType, orgId) {
      var path = appType === 'site' ? '' : '/organizations/' + orgId;
      return _choerodonBootCombine.axios.put('notify/v1/notices/letters/templates/' + id + path, JSON.stringify(data));
    };
  }

  (0, _createClass3['default'])(InMailTemplateStore, [{
    key: 'setLoading',
...
var mailTemplateStore = new InMailTemplateStore();
