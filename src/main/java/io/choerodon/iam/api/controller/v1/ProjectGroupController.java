...
public class ProjectGroupController {
    ...
    @Permission(level = ResourceLevel.ORGANIZATION, roles = {InitRoleCode.ORGANIZATION_ADMINISTRATOR})
    @ApiOperation(value = "项目群下批量添加子项目")
    @PostMapping
    public ResponseEntity<List<ProjectGroupDTO>> create(@RequestBody @Valid List<ProjectGroupDTO> projectGroupDTOList) {
        if (projectGroupDTOList == null || projectGroupDTOList.isEmpty()) {
            throw new CommonException("error.add.project.group.empty");
        }
        ...
        return new ResponseEntity<>(projectGroupService.batchAddProjsToTheGroup(projectGroupDTOList), HttpStatus.OK);
    }
    ...
    @Permission(level = ResourceLevel.ORGANIZATION, roles = {InitRoleCode.ORGANIZATION_ADMINISTRATOR})
    @ApiOperation(value = "更新项目群下项目(开始与结束时间)")
    @PutMapping
    public ResponseEntity<ProjectGroupDTO> update(@RequestBody ProjectGroupDTO projectGroupDTO) {
        if (projectGroupDTO.getStartDate() == null) {
            projectGroupDTO.setStartDate(new Date());
        }
        if (projectGroupDTO.getStartDate() != null && projectGroupDTO.getEndDate() != null
        ...
        return new ResponseEntity<>(projectGroupService.updateGroupDate(projectGroupDTO), HttpStatus.OK);
    }
}
